...
int _Random_grid(int **grid_info, int **adj_cid, int **road_cross, int **town_cross, int **river_cross, double *p_road, double *p_town, double *p_river, double *p_fence, int week_cont, int current_cid);
int _Natal_migration(int **grid_info, int **adj_cid, int **n_wb, int **road_cross, int **town_cross, int **river_cross, double *p_road, double *p_town, double *p_river, double *p_fence, int week_cont, int current_cid, int sex);
...
void _Simulate_ASF(int **grid_info, int **adj_cid, double **grid_xy, int **reach_cid, double **reach_dist, int **n_wb, int **n_wb_tmp, int **carcass, int **carcass_tmp, int **w_demo, 
...
double _Run_the_model(int **grid_info, int **adj_cid, double **grid_xy, int **reach_cid, double **reach_dist, int **SS_n, int **SS_k, int **SS_n_h, int **SS_k_h, 
...
//				printf("IDX %i: %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f\n", val, _parameters[val][0], _parameters[val][1], _parameters[val][2], _parameters[val][3], _parameters[val][4], _parameters[val][5], _parameters[val][6]);
				ll= _Run_the_model(grid_info, adj_cid, grid_xy, reach_cid, reach_dist, SS_n, SS_k, SS_n_h, SS_k_h, road_cross, town_cross, river_cross, beta_wb, beta_car, pi_between, p_road, p_town, p_river, p_fence, alpha);
//				printf("IDX %i: %4.3f\n", val, ll);
			...
			if (_Random_uniform(0.0, 1.0) < prob && loglikelihood[1] != 0.0) {
...
// Probability density function of normal distribution (FOR sd = 2.0!!!!)
double _PDF_normal(double x, double mu) {
	double val, sd= 2.0;
...
int _Random_grid(int **grid_info, int **adj_cid, int **road_cross, int **town_cross, int **river_cross, double *p_road, double *p_town, double *p_river, double *p_fence, int week_cont, int current_cid) {
	...
	// Extract the probability of migration
	for (i= 0; i < n_adj; ++i) {
		j= adj_cid[current_cid][i];
		if (j != _na) {
			prob[i]= _Cross_cell(grid_info, road_cross, town_cross, river_cross, p_road, p_town, p_river, p_fence, week_cont, current_cid, j);
		} else {
			prob[i]= 0.0;
		}
	}
	...
	// In case of the current cell is surrounded by nothing
	if (x == 0.0) {
		return current_cid;
	// Otherwise,
	} else {
...
int _Natal_migration(int **grid_info, int **adj_cid, int **n_wb, int **road_cross, int **town_cross, int **river_cross, double *p_road, double *p_town, double *p_river, double *p_fence, int week_cont, int current_cid, int sex) {
	...
	if (sex == 0) {
		while (step < d_lim) {
			tmp_cid= _Random_grid(grid_info, adj_cid, road_cross, town_cross, river_cross, p_road, p_town, p_river, p_fence, week_cont, tmp_cid);
...
// ASF simulation for THREE weeks (as the summary statistics is based on the number of carcass and it takes 3 weeks for an exposed WB to die due to ASF)
void _Simulate_ASF(int **grid_info, int **adj_cid, double **grid_xy, int **reach_cid, double **reach_dist, int **n_wb, int **n_wb_tmp, int **carcass, int **carcass_tmp, int **w_demo, 
	int **SS_n, int **SS_k, int **SS_grid, int **SS_n_h, int **SS_k_h, int **road_cross, int **town_cross, int **river_cross,
	int week_cont, double *beta_wb, double *beta_car, double *pi_between, double *p_road, double *p_town, double *p_river, double *p_fence, double *alpha, double *log_lik) {
...
//	int safeguard= 0;
	...
	if (week == w_update || week_cont == 0) {
		for (i= 0; i < n_grid; ++i) {
			...
			do {
				val= (int)round(_Random_normal(50.5, 3.0));
				val %= 52;
			...
			do {
				val= (w_demo[i][0] + (int)round(_Random_uniform(0.0, disp))) % 52;
			} while (val == w_update);
	...
	for (j= 0; j < n_grid; ++j) {
		...
		for (k= 0; k < g_dz; ++k) {
			// Carcass degradation
			val= _Random_binomial(carcass[j][k], 1.0 - exp(-1.0 / w_carcass));
			carcass_tmp[j][k]= carcass[j][k] - val;

			if (k == _i) {
				val= _Random_binomial(carcass_tmp[j][k], 1.0 - exp(-1.0 / w_asf_carcass));
			...
			if (d_fence == 1) {
				...
				val= _Random_binomial(n_wb_tmp[j][g_demo * k + _fp], p_mort);
				n_wb_tmp[j][g_demo * k + _fp] -= val; n_fp -= val;

				val= _Random_binomial(n_wb_tmp[j][g_demo * k + _fy], p_mort);
				n_wb_tmp[j][g_demo * k + _fy] -= val; n_fy -= val;

				val= _Random_binomial(n_wb_tmp[j][g_demo * k + _sow], p_mort);
				n_wb_tmp[j][g_demo * k + _sow] -= val; n_sow -= val;

				val= _Random_binomial(n_wb_tmp[j][g_demo * k + _mp], p_mort);
				n_wb_tmp[j][g_demo * k + _mp] -= val; n_mp -= val;

				val= _Random_binomial(n_wb_tmp[j][g_demo * k + _my], p_mort);
				n_wb_tmp[j][g_demo * k + _my] -= val; n_my -= val;

				val= _Random_binomial(n_wb_tmp[j][g_demo * k + _boar], p_mort);
			...
			if (week == w_demo[j][col_disp]) {
				...
				new_cid= _Natal_migration(grid_info, adj_cid, n_wb, road_cross, town_cross, river_cross, p_road, p_town, p_river, p_fence, week_cont, j, 0);
				...
				if (new_cid != j) {
					if (n_sow > grid_info[j][col_sow] && n_fp != 0) {
						...
						if (n_fp > grid_info[new_cid][col_sow]) {
							n_move= _Random_binomial(grid_info[new_cid][col_sow], p_move);
				...
				while (n_wb_tmp[j][g_demo * k + _my] > 0) {
					new_cid= _Natal_migration(grid_info, adj_cid, n_wb, road_cross, town_cross, river_cross, p_road, p_town, p_river, p_fence, week_cont, j, 1);
			...
			if (week == w_demo[j][col_farrow]) {
				...
				new_born= (int)((int)_Random_normal(5.0, 1.0) * _Random_binomial(n_wb_tmp[j][g_demo * k + _sow], p_repro)) / 2; // Male/female newborn piglet
				if (k == _r) {
		...
		for (k= 0; k < g_demo; ++k) {
			// M -> S
			dM= _Random_binomial(n_wb_tmp[j][g_demo * _m + k], 1.0 - exp(-1.0 / w_mab));
			// S -> E
			dS= _Random_binomial(n_wb_tmp[j][g_demo * _s + k], lambda);
			// E -> I
			dE= n_wb_tmp[j][g_demo * _e + k];
			// I -> R or death
			dI= n_wb_tmp[j][g_demo * _i + k];
			dC= _Random_binomial(dI, p_death); dR= dI - dC;
...
*/
	// Run the simulation
	for (i= 0; i < w_SS; ++i) {
		_Simulate_ASF(grid_info, adj_cid, grid_xy, reach_cid, reach_dist, n_wb, n_wb_tmp, carcass, carcass_tmp, w_demo, SS_n, SS_k, SS_grid, SS_n_h, SS_k_h, road_cross, town_cross, river_cross, i, 
